# -*- coding: utf-8 -*-
from eggs import deploy_ip

from .env.dev import DEV_NEWS_HOST, DEV_NEWS_PORT, DEV_NEWS_DB, DEV_NEWS_TABLE, DEV_CRAWLER_NEWS_TABLE
from .env.dev import DEV_LOG_HOST, DEV_LOG_PORT, DEV_LOG_DB, DEV_LOG_TABLE
from .env.dev import DEV_CORPUS_HOST, DEV_CORPUS_PORT, DEV_CORPUS_DB, DEV_CORPUS_TABLES
from .env.dev import DEV_A_POST_HOST, DEV_A_POST_PORT, DEV_A_POST_DB, DEV_A_POST_TABLE
from .env.dev import DEV_SCRAPYD_HOST
from .env.dev import DEV_SH_HOST, DEV_SH_PORT, DEV_SH_ADA_DB, DEV_SH_NEWS_DB, DEV_SH_TABLES
from .env.dev import DEV_GRAPH_CRAWLER_DB, DEV_GRAPH_CRAWLER_TABLE

from .env.pro import PRO_NEWS_HOST, PRO_NEWS_PORT, PRO_NEWS_DB, PRO_NEWS_TABLE, PRO_CRAWLER_NEWS_TABLE
from .env.pro import PRO_LOG_HOST, PRO_LOG_PORT, PRO_LOG_DB, PRO_LOG_TABLE
from .env.pro import PRO_CORPUS_HOST, PRO_CORPUS_DB, PRO_CORPUS_PORT, PRO_CORPUS_TABLES
from .env.pro import PRO_A_POST_HOST, PRO_A_POST_DB, PRO_A_POST_PORT, PRO_A_POST_TABLE
from .env.pro import PRO_SCRAPYD_HOST
from .env.pro import PRO_SH_HOST, PRO_SH_PORT, PRO_SH_ADA_DB, PRO_SH_NEWS_DB, PRO_SH_TABLES
from .env.pro import PRO_GRAPH_CRAWLER_DB, PRO_GRAPH_CRAWLER_TABLE

from .base.name import SITE_NAME_MAP, STOCK_CAT, ORIGIN_CATEGORY, NEWS_SOURCE

_PRO_IP = []  # Development Environment
_DEV_IP = ['192.168.1.22', '192.168.100.20', '192.168.250.207']  # Test Environment

if deploy_ip in _DEV_IP:
    NEWS_HOST = DEV_NEWS_HOST
    NEWS_PORT = DEV_NEWS_PORT
    NEWS_DB = DEV_NEWS_DB
    NEWS_TABLE = DEV_NEWS_TABLE
    CRAWLER_NEWS_TABLE = DEV_CRAWLER_NEWS_TABLE

    LOG_HOST = DEV_LOG_HOST
    LOG_PORT = DEV_LOG_PORT
    LOG_DB = DEV_LOG_DB
    LOG_TABLE = DEV_LOG_TABLE

    CORPUS_HOST = DEV_CORPUS_HOST
    CORPUS_PORT = DEV_CORPUS_PORT
    CORPUS_DB = DEV_CORPUS_DB
    CORPUS_TABLES = DEV_CORPUS_TABLES

    # 测试环境下A股公告的mongo配置
    A_POST_HOST = DEV_A_POST_HOST
    A_POST_PORT = DEV_A_POST_PORT
    A_POST_DB = DEV_A_POST_DB
    A_POST_TABLE = DEV_A_POST_TABLE

    # Scrapyd Server Host
    SCRAPYD_HOST = DEV_SCRAPYD_HOST

    # SH count
    SH_CORPUS_HOST = DEV_SH_HOST
    SH_CORPUS_PORT = DEV_SH_PORT
    SH_CORPUS_ADA = DEV_SH_ADA_DB
    SH_CORPUS_NEWS = DEV_SH_NEWS_DB
    SH_CORPUS_TABLES = DEV_SH_TABLES

    # SH iwencai site
    GRAPH_CRAWLER_DB = DEV_GRAPH_CRAWLER_DB
    GRAPH_CRAWLER_TABLE = DEV_GRAPH_CRAWLER_TABLE
else:
    NEWS_HOST = PRO_NEWS_HOST
    NEWS_PORT = PRO_NEWS_PORT
    NEWS_DB = PRO_NEWS_DB
    NEWS_TABLE = PRO_NEWS_TABLE
    CRAWLER_NEWS_TABLE = PRO_CRAWLER_NEWS_TABLE

    LOG_HOST = PRO_LOG_HOST
    LOG_PORT = PRO_LOG_PORT
    LOG_DB = PRO_LOG_DB
    LOG_TABLE = PRO_LOG_TABLE

    CORPUS_HOST = PRO_CORPUS_HOST
    CORPUS_PORT = PRO_CORPUS_PORT
    CORPUS_DB = PRO_CORPUS_DB
    CORPUS_TABLES = PRO_CORPUS_TABLES

    # 生产环境下A股公告的mongo配置
    A_POST_HOST = PRO_A_POST_HOST
    A_POST_PORT = PRO_A_POST_PORT
    A_POST_DB = PRO_A_POST_DB
    A_POST_TABLE = PRO_A_POST_TABLE

    # Scrapyd Server Host
    SCRAPYD_HOST = PRO_SCRAPYD_HOST

    # SH count
    SH_CORPUS_HOST = PRO_SH_HOST
    SH_CORPUS_PORT = PRO_SH_PORT
    SH_CORPUS_ADA = PRO_SH_ADA_DB
    SH_CORPUS_NEWS = PRO_SH_NEWS_DB
    SH_CORPUS_TABLES = PRO_SH_TABLES

    # SH iwencai site
    GRAPH_CRAWLER_DB = PRO_GRAPH_CRAWLER_DB
    GRAPH_CRAWLER_TABLE = PRO_GRAPH_CRAWLER_TABLE
